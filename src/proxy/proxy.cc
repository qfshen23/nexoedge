...
Proxy::Proxy(ProxyCoordinator *coordinator, std::map<int, std::string> *map, BgChunkHandler::TaskQueue *queue,
             DeduplicationModule *dedup, bool enableAutoRepair) {
  // config
  ...
  // auto file recovery
  _ongoingRepairCnt = 0;
  if (enableAutoRepair) pthread_create(&_rt, NULL, Proxy::backgroundRepair, this);

  if (config.ackRedundancyInBackground()) pthread_create(&_tct, NULL, Proxy::backgroundTaskCheck, this);
...
Proxy::~Proxy() {
  ...
  if (Config::getInstance().autoFileRecovery()) pthread_join(_rt, NULL);
  if (Config::getInstance().ackRedundancyInBackground()) pthread_join(_tct, NULL);
...
int Proxy::getAgentStatus(ProxyCoordinator::AgentInfo **info) { return _coordinator->getAgentStatus(info); }

bool Proxy::getProxyStatus(SysInfo &info) { return _coordinator->getProxyStatus(info); }
...
void *Proxy::backgroundRepair(void *arg) {
  ...
  while (self->_running && (pollIntv > 0 || fileScanIntv > 0 || chunkScanIntv > 0)) {
    ...
    if ((fileScanIntv > 0 && lastFileScan + fileScanIntv <= curTime) ||
      ...
      FileInfo *list = 0;
      int numFiles = self->getFileList(&list, /* withSize */ true, /* withTime */ true,
                                       /* withVersions */ true);
...
#define checkFile(__FL__)                                                                                              \
  do {                                                                                                                 \
    ...
    if (fileScanIntv > 0 && lastFileScan + fileScanIntv <= curTime &&                                                  \
      ...
      DLOG(INFO) << "Add file " << __FL__.name << " of version " << __FL__.version << " for missing chunk at "         \
                 << time(NULL);                                                                                        \
    }                                                                                                                  \
...
#undef checkFile
      ...
      if (lastFileScan + fileScanIntv <= time(NULL)) lastFileScan = time(NULL);
      if (lastChunkScan + chunkScanIntv <= time(NULL)) lastChunkScan = time(NULL);
      delete[] list;
    }
...
bool Proxy::needsRepair(File &f, bool updateStatusFirst) {
  ...
  return _coordinator->checkContainerLiveness(rf.containerIds, rf.numChunks, chunkIndices, updateStatusFirst,
                                              /* checkAllFailures */ false) > 0 &&
         rf.mtime + Config::getInstance().getFileRecoverInterval() < time(NULL);
}

bool Proxy::batchedChunkScan(const FileInfo *list, const int numFiles, const int curIdx, int &numChunksInBatch,
                             int &batchStartIdx) {
  // report error if list is not provided, curIdx is beyond the list, or
  // batchStartIdx is beyond the list
  if (list == NULL || curIdx >= numFiles || batchStartIdx >= numFiles) return false;
  ...
  if (curIdx + 1 == numFiles || numChunksInBatch >= batchSizeLimit) {
    DLOG(INFO) << "File list num files = " << numFiles << " curidx = " << curIdx
               << " numChunksBatched = " << numChunksInBatch << " batchStart = " << batchStartIdx
    ...
    for (int fidx = batchStartIdx; fidx <= curIdx; fidx++) {
      ...
      if (list[fidx].mtime + config.getFileRecoverInterval() > time(NULL)) continue;
      ...
      if (samplingPolicy == ChunkScanSamplingPolicy::FILE_LEVEL) {
        if (Util::includeSample(curIdx - batchStartIdx + 1,
          ...
          DLOG(INFO) << "Sampling: skip 1 file " << list[fidx].name << ", cur sampling rate = " << numSampled << "/"
      ...
      for (int cidx = 0; cidx < f->numChunks; cidx++) {                                                               \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::CHUNK_LEVEL) {                                                 \
          if (Util::includeSample(numChunksInBatch, samplingRate)) {                                                  \
            ...
            DLOG(INFO) << "Sampling: skip 1 chunk in file " << f->name << ", cur sampling rate = " << numSampled      \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::STRIPE_LEVEL) {                                                \
          ...
          if (chunkInStripeIdx == 0) numSampled = 0;                                                                  \
          if (Util::includeSample(f->codingMeta.n, samplingRate)) {                                                   \
            numSampled++;                                                                                             \
          } else { /* skip chunk (in stripe) */                                                                       \
            DLOG(INFO) << "Sampling: skip 1 chunk in stripe " << cidx / f->codingMeta.n << " of file " << f->name     \
                       << ", cur sampling rate = " << numSampled << "/" << f->codingMeta.n << " vs " << samplingRate; \
            continue;                                                                                                 \
          }                                                                                                           \
        }                                                                                                             \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::CONTAINER_LEVEL) {                                             \
          ...
          if (!Util::includeSample(numChunksInBatch / _coordinator->getNumAliveContainers(), samplingRate)) {         \
            DLOG(INFO) << "Sampling: skip 1 chunk in container " << containerId                                       \
                       << ", cur sampling rate = " << cf.numChunks << "/"                                             \
                       << numChunksInBatch / _coordinator->getNumAliveContainers() << " vs " << samplingRate;         \
            continue;                                                                                                 \
          }                                                                                                           \
        }                                                                                                             \
        ...
        if (cf.numChunks < numChunksInBatch) {                                                                        \
          ...
          LOG(WARNING) << "Skipping chunk " << cidx << " of file " << f->name << " as the chunk list of container "   \
                       << it->second << " is full (" << cf.numChunks << "/" << numChunksInBatch << ")";               \
        }                                                                                                             \
      }                                                                                                               \
      f = 0;                                                                                                          \
    }                                                                                                                 \
...
#undef checkChunks
    ...
    for (auto it : containerId2vfMap) {
      ...
      for (int cidx = 0; numFailedChunks > 0 && cidx < cf.numChunks; cidx++) {
        ...
        if (!chunkIndicators[cidx]) continue;
    ...
    // release file lock
    for (auto &it : fileMap) {
      if (it.second.second) unlockFile(*(it.second.first));
...
void *Proxy::backgroundTaskCheck(void *arg) {
  ...
  while (self->_running) {
    ...
    if (numTask > 0) LOG(INFO) << "------- Background Task Progress -------";
    for (int i = 0; i < numTask; i++)
      LOG(INFO) << " " << std::left << std::setfill(' ') << std::setw(30) << task[i] << ": " << progress[i] << "%";
    if (numTask > 0) LOG(INFO) << "----------------------------------------";
    delete[] task;
    delete[] progress;
        ...
        for (int i = 0; i < file.numStripes; i++) {
          ...
          if (numFailed > 0) {
            ...
            for (int j = 0; j < numChunksPerStripe; j++) {
              // skip if not failed
              if (chunkIndicator[j]) continue;
              scf.containerIds[j] = -1;
            }
            int ret = self->_metastore->updateChunks(scf, file.version);
            // update metadata in metastore
            if (ret != 0) {
              LOG(ERROR) << "Failed to mark chunks that needs to be repaired for file " << scf.name << " in stripe "
                         << i << " error = " << ret;
  ...
  while (self->_running && reqCheckIntv > 0) {
    ...
    for (int fidx = 0; fidx < numFiles; fidx++) {
      ...
      for (int ridx = 0; ridx < numRecords; ridx++) {
        ...
        if (self->_coordinator->checkContainerLiveness(&containerId, 1, &online, /* updateStatusFirst */ false) > 0)
          continue;
        ...
        if (isWrite) {
          ...
          if (!removeJournal) {
            ...
            if (removeJournal) {
              DLOG(INFO) << "Update the valid chunk " << chunkId << " of file " << fileMeta.name << " of versoin "
                         << fileMeta.version << " in container " << containerId << " in the file metadata.";
              ...
              LOG(INFO) << "Going to delete the invalid chunk " << reqFileMeta.chunks[0].getChunkName() << " of file "
                        << reqFileMeta.name << " of versoin " << reqFileMeta.version << " in container "
          ...
          // succeeded
          LOG(INFO) << "Going to delete the chunk " << chunkId << " of file " << fileMeta.name << " of versoin "
                    << fileMeta.version << " in container " << containerId << " again.";
          bool chunkExistsAndMatches = fileMeta.numChunks > chunkId && fileMeta.containerIds[chunkId] == containerId &&
        ...
        if (removeJournal) {
          if (!self->_metastore->updateChunkInJournal(fileMeta, chunk, isWrite, /* deleteRecord */ removeJournal,
                                                      containerId)) {
            LOG(WARNING) << "Failed to remove the chunk journal of file " << file.name << " chunk " << chunkId
                         << " in namespace " << file.namespaceId << " version " << file.version << ".";
          }
        }
      }
...
void *Proxy::stagingBGWrite(void *param) {
  ...
  while (self->_running && scanIntv > 0) {
    ...
    if (bgwritePolicy == "idle") {
      ...
      for (unsigned int i = 0; i < (unsigned int)info.cpu.num; i++) avgCPUUsage += info.cpu.usage[i];

      // skip background write if the system is busy
      // if (!(avgCPUUsage / info.cpu.num < 50) && (info.mem.free * 1.0 /
      // info.mem.total < 0.5)) {
      if (avgCPUUsage / info.cpu.num >= 50) continue;
...
bool Proxy::bgwriteFileToCloud(File &f) {
  ...
  if (!staged || rfinfo.mtime + scanIntv * 2 > time(NULL)) {
    ...
    LOG(WARNING) << BG_WRITE_TO_CLOUD_TAG << "Skip " << (staged ? "recently modified" : "non-existing") << " file "
                 << rf.name;
    return false;
  }
  ...
  // get stripe size
  unsigned long stripeSize =
      _chunkManager->getMaxDataSizePerStripe(codingMeta.coding, codingMeta.n, codingMeta.k, codingMeta.maxChunkSize);
  ...
  LOG(INFO) << "Read file " << f.name
            << " from staging, (data) speed = " << (wf.length * 1.0 / (1 << 20)) / (duration.wall * 1.0 / 1e9)
            << " MB/s "
  ...
  LOG_IF(INFO, duration.wall > 0) << "Write back file " << f.name << " to cloud, (data) speed = "
                                  << (wf.length * 1.0 / (1 << 20)) / (duration.wall * 1.0 / 1e9) << " MB/s "
                                  << "(" << wf.size * 1.0 / (1 << 20) << "MB in " << (duration.wall * 1.0 / 1e9)
                                  << " seconds)";
  ...
  LOG(INFO) << "Write back file " << f.name << ", (meta) duration = " << (mytimer.elapsed().wall * 1.0 / 1e6)
            << " millseconds)";
...
std::map<std::string, double> Proxy::genStatsMap(const boost::timer::cpu_times &dataT,
