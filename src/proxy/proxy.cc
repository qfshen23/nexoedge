...
#include "dedup/impl/dedup_none.hh"
#include "proxy.hh"
...
Proxy::Proxy(ProxyCoordinator *coordinator, std::map<int, std::string> *map,
             BgChunkHandler::TaskQueue *queue, DeduplicationModule *dedup,
  ...
  _chunkManager =
      new ChunkManager(_containerToAgentMap, _io, _bgChunkHandler, _metastore);
  _repairChunkManager = new ChunkManager(_containerToAgentMap, _repairio,
                                         _bgChunkHandler, _metastore);
  _tcChunkManager =
      new ChunkManager(_containerToAgentMap, _tcio, _bgChunkHandler);
  ...
  if (_stagingEnabled) {
    ...
    // pthread_create(&_stagingBGCacheReadWorker, 0, stagingBGCacheReads,
    // (void*) this); _stagingPendingReadCache = new
    // RingBuffer<File>(config.getReadCacheBufferSize(), /* block on empty */
    // true, 1, /* block on full */ false);
  }
}
...
void Proxy::updateAgentStatus() { _coordinator->updateAgentStatus(); }
...
void Proxy::getStorageUsage(unsigned long int &usage,
...
void *Proxy::backgroundRepair(void *arg) {
  ...
  while (self->_running &&
         (pollIntv > 0 || fileScanIntv > 0 || chunkScanIntv > 0)) {
    time_t curTime = time(NULL);
    // scan at certain interval for files to repair
    if ((fileScanIntv > 0 && lastFileScan + fileScanIntv <= curTime) ||
        (chunkScanIntv > 0 && lastChunkScan + chunkScanIntv <= curTime)) {
      DLOG(INFO) << "Start scanning at " << time(NULL);
      // scan all file names for repair
      FileInfo *list = 0;
      int numFiles =
          self->getFileList(&list, /* withSize */ true, /* withTime */ true,
                            /* withVersions */ true);
      int batchStartIdx = 0, numChunksInBatch = 0;
      File file;

#define checkFile(__FL__)                                                      \
  do {                                                                         \
    DLOG(INFO) << "Check file " << file.name << " version " << file.version    \
               << " for missing chunk at " << time(NULL);                      \
    if (fileScanIntv > 0 && lastFileScan + fileScanIntv <= curTime &&          \
        self->needsRepair(__FL__, /* updateStatusFirst */ i == 0)) {           \
      self->_metastore->markFileAsNeedsRepair(__FL__);                         \
      DLOG(INFO) << "Add file " << __FL__.name << " of version "               \
                 << __FL__.version << " for missing chunk at " << time(NULL);  \
    }                                                                          \
    /* scan for corrupted chunks */                                            \
    if (chunkScanIntv > 0 && lastChunkScan + chunkScanIntv <= curTime) {       \
      self->batchedChunkScan(list, numFiles, i, numChunksInBatch,              \
                             batchStartIdx);                                   \
    }                                                                          \
  } while (0)
      ...
      if (self->_coordinator->getNumAliveContainers(/* skipfull */ true) >= k) {
        ...
        do {
          ...
          for (int i = 0; i < numToRepair; i++) {
            // if is locked for repair and repair suceed, remove from under
            // repair list; otherwise, put it back to the list for pending
            // repair (retry)
            if (self->repairFile(files[i], /* isBg */ true)) {
              DLOG(INFO) << "Repair file " << files[i].name << " at "
                         << time(NULL);
            } else {
...
#define updateTimeToSleep(__LAST_ACT_TIME__, __ACT_INTV__)                     \
  do {                                                                         \
    ...
    if (timeToNextAction > 0 || (timeToNextAction == 0 && __ACT_INTV__ > 0)) { \
      ...
      sleepTime = __ACT_INTV__ > 0 ? std::min(sleepTime, (time_t)__ACT_INTV__) \
                                   : sleepTime;                                \
    }                                                                          \
  } while (0)
...
bool Proxy::needsRepair(File &f, bool updateStatusFirst) {
  ...
  // recover if there are chunk failures, and the file has not been modified
  // since last repair check
  return _coordinator->checkContainerLiveness(
             rf.containerIds, rf.numChunks, chunkIndices, updateStatusFirst,
             /* checkAllFailures */ false) > 0 &&
         rf.mtime + Config::getInstance().getFileRecoverInterval() < time(NULL);
}

bool Proxy::batchedChunkScan(const FileInfo *list, const int numFiles,
                             const int curIdx, int &numChunksInBatch,
                             int &batchStartIdx) {
  // report error if list is not provided, curIdx is beyond the list, or
  // batchStartIdx is beyond the list
  ...
  // DLOG(INFO) << "File list num files = " << numFiles << " curidx = " <<
  // curIdx << " numChunksBatched = " << numChunksInBatch << " batchStart = " <<
  // batchStartIdx << " batchSizeLimit = " << batchSizeLimit;

  // issue batched chunk checking request if we reach the end of list or the
  // numbers of batched chunks reaches the limit
  if (curIdx + 1 == numFiles || numChunksInBatch >= batchSizeLimit) {
    DLOG(INFO) << "File list num files = " << numFiles << " curidx = " << curIdx
               << " numChunksBatched = " << numChunksInBatch
               << " batchStart = " << batchStartIdx
               << " batchSizeLimit = " << batchSizeLimit;

    File vf[MAX_NUM_CONTAINERS]; // virtual files for chunks associated to each
                                 // container
    std::map<int, int>
        containerId2vfMap; // mapping from container id to vf index
    std::map<std::pair<boost::uuids::uuid, int>, std::pair<File *, bool>>
        fileMap;
    std::set<std::pair<boost::uuids::uuid, int>> filesModified;

    int numChunksProcessed[MAX_NUM_CONTAINERS]; // counter for number of chunks
                                                // processed by container
                                                // (chunk-level/container-level
                                                // sampling)
    int samplingPolicy = config.getChunkScanSamplingPolicy();
    double samplingRate = config.getChunkScanSamplingRate();
    int numSampled =
        0; // number of chunks/files sampled (chunk-level/file-level sampling)

    // batch chunks from files into virtual files according to their container
    // ids
    for (int fidx = batchStartIdx; fidx <= curIdx; fidx++) {
      // skip files that are recently modified
      // DLOG(INFO) << "File = " << fidx << " mtime = " << list[fidx].mtime << "
      // now = " << time(NULL) << " interval = " <<
      // config.getFileRecoverInterval();
      ...
      if (samplingPolicy == ChunkScanSamplingPolicy::FILE_LEVEL) {
        if (Util::includeSample(curIdx - batchStartIdx + 1,
                                samplingRate)) { // include this in the samples
          numSampled++;
        } else { // skip this
          DLOG(INFO) << "Sampling: skip 1 file " << list[fidx].name
                     << ", cur sampling rate = " << numSampled << "/"
                     << curIdx - batchStartIdx + 1 << " vs " << samplingRate;
          continue;
        }
      }

#define checkChunks(__SKIP_LOCK__)                                             \
  do {                                                                         \
    /* lock and get metadata of all files in the batch */                      \
    if ((__SKIP_LOCK__ && _metastore->getMeta(*f)) ||                          \
        (!__SKIP_LOCK__ && lockFileAndGetMeta(*f, "chunk scan"))) {            \
      /* save file metadata */                                                 \
      fileMap.insert(std::make_pair(std::make_pair(f->uuid, f->version),       \
                                    std::make_pair(f, !__SKIP_LOCK__)));       \
      ...
      for (int cidx = 0; cidx < f->numChunks; cidx++) {                        \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::CHUNK_LEVEL) {          \
          if (Util::includeSample(numChunksInBatch, samplingRate)) {           \
            ...
            DLOG(INFO) << "Sampling: skip 1 chunk in file " << f->name         \
                       << ", cur sampling rate = " << numSampled << "/"        \
                       << numChunksInBatch << " vs " << samplingRate;          \
            continue;                                                          \
          }                                                                    \
        }                                                                      \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::STRIPE_LEVEL) {         \
          ...
          if (Util::includeSample(f->codingMeta.n, samplingRate)) {            \
            ...
            DLOG(INFO) << "Sampling: skip 1 chunk in stripe "                  \
                       << cidx / f->codingMeta.n << " of file " << f->name     \
                       << ", cur sampling rate = " << numSampled << "/"        \
                       << f->codingMeta.n << " vs " << samplingRate;           \
            continue;                                                          \
          }                                                                    \
        }                                                                      \
        ...
        std::tie(it, std::ignore) = containerId2vfMap.insert(                  \
            std::make_pair(containerId, containerId2vfMap.size()));            \
        if (it == containerId2vfMap.end()) {                                   \
          LOG(WARNING) << "Failed to process check for chunk " << cidx         \
                       << " of file " << f->name;                              \
          continue;                                                            \
        }                                                                      \
        ...
        if (samplingPolicy == ChunkScanSamplingPolicy::CONTAINER_LEVEL) {      \
          ...
          /* assume the chunks are evenly distributed to all existing          \
           * containers */                                                     \
          /* TODO remove this assumption (num. of containers can be larger     \
           * than n) */                                                        \
          if (!Util::includeSample(numChunksInBatch /                          \
                                       _coordinator->getNumAliveContainers(),  \
                                   samplingRate)) {                            \
            DLOG(INFO) << "Sampling: skip 1 chunk in container "               \
                       << containerId                                          \
                       << ", cur sampling rate = " << cf.numChunks << "/"      \
                       << numChunksInBatch /                                   \
                              _coordinator->getNumAliveContainers()            \
                       << " vs " << samplingRate;                              \
            continue;                                                          \
        ...
        if (cf.numChunks == 0) {                                               \
          try {                                                                \
            ...
            LOG(ERROR) << "Failed to allocate memory for " << numChunksInBatch \
                       << " chunks in batch for checking";                     \
            f = 0;                                                             \
            /*return false;*/                                                  \
            break;                                                             \
          }                                                                    \
        ...
        if (cf.numChunks < numChunksInBatch) {                                 \
          ...
          LOG(WARNING) << "Skipping chunk " << cidx << " of file " << f->name  \
                       << " as the chunk list of container " << it->second     \
                       << " is full (" << cf.numChunks << "/"                  \
                       << numChunksInBatch << ")";                             \
        }                                                                      \
      }                                                                        \
      f = 0;                                                                   \
      ...
      for (int vi = 0; vi < numVersions; vi++) {
        ...
        DLOG(INFO) << "Check chunks of file " << f->name << " version "
                   << f->version;
        checkChunks(true);
      }
...
#undef checkChunks
    ...
    // issue batched chunk request (of virtual files) and update metadata of
    // files to repair
    for (auto it : containerId2vfMap) {
      // skip checking if container already fails
      bool containerStatus = false;
      int containerId = it.first;
      if (_coordinator->checkContainerLiveness(&containerId, 1,
                                               &containerStatus) > 0) {
      ...
      int numFailedChunks =
          _chunkManager->verifyFileChecksums(cf, chunkIndicators);
      ...
      for (int cidx = 0; numFailedChunks > 0 && cidx < cf.numChunks; cidx++) {
        ...
        try {
          std::pair<boost::uuids::uuid, int> key = std::make_pair(
              cf.chunks[cidx].fuuid, cf.chunks[cidx].fileVersion);
          ...
          DLOG(INFO) << "Chunk corruption detected, file " << f->name
                     << " chunk " << chunkId;
    ...
    // update modified file metadata in metastore and mark them as pending for
    // repair
    for (auto idx : filesModified) {
      File *f = fileMap.at(idx).first;
      _metastore->putMeta(*f);
      _metastore->markFileAsNeedsRepair(*f);
      DLOG(INFO) << "Add file " << f->name
                 << " for repairing corrupted chunks at " << time(NULL);
    }
...
int Proxy::checkCorruptedChunks(bool *chunksCorrupted, int numChunks,
                                bool *chunkIndicator) {
...
void *Proxy::backgroundTaskCheck(void *arg) {
  ...
  while (self->_running) {
    ...
    for (int i = 0; i < numTask; i++)
      LOG(INFO) << " " << std::left << std::setfill(' ') << std::setw(30)
                << task[i] << ": " << progress[i] << "%";
    if (numTask > 0)
      LOG(INFO) << "----------------------------------------";
    delete[] task;
    ...
    while (self->_metastore->getNextFileForTaskCheck(file)) {
      ...
      // lock file for checking chunk existence and update any failed chunk
      // locations if no task in queue
      if (self->_metastore->getMeta(file) && file.numStripes > 0) {
        ...
        for (int i = 0; i < file.numStripes; i++) {
          ...
          if (numFailed > 0) {
            ...
            if (ret != 0) {
              LOG(ERROR)
                  << "Failed to mark chunks that needs to be repaired for file "
                  << scf.name << " in stripe " << i << " error = " << ret;
  ...
  while (self->_running && reqCheckIntv > 0) {
    ...
    for (int fidx = 0; fidx < numFiles; fidx++) {
      ...
      if (!self->lockFile(fileMeta)) {
        LOG(WARNING) << "Failed to lock file " << file.name
                     << " for checking incomplete chunk requests.";
        continue;
      }
      ...
      std::vector<std::tuple<Chunk, int /* container id*/, bool /* isWrite */,
                             bool /* isPre */>>
          records;
      ...
      for (int ridx = 0; ridx < numRecords; ridx++) {
        ...
        // skip on-going operations (this should not happen as on-going
        // operation should have locked the file?)
        if (isPre)
          continue;

        // skip requests on offline containers
        if (self->_coordinator->checkContainerLiveness(
                &containerId, 1, &online, /* updateStatusFirst */ false) > 0)
          continue;

        // copy the chunk metadata to prepare for a request
        reqFileMeta.genUUID();
        reqFileMeta.chunks[0].copyMeta(chunk);
        reqFileMeta.chunks[0].setId(reqFileMeta.namespaceId, reqFileMeta.uuid,
                                    chunkId);
        ...
        if (isWrite) {
          // check if the chunk record in the file metadata is same as the
          // journaled one if true, simply treat the request as committed and
          // remove it from the journal
          removeJournal = fileMeta.numChunks > chunkId &&
                          chunk.matchMeta(fileMeta.chunks[chunkId]);
          // otherwise, check chunk existance and integrity
          if (!removeJournal) {
            removeJournal = self->_tcChunkManager->verifyFileChecksums(
                                reqFileMeta, chunkIndicators) == 0;
            if (removeJournal) {
              DLOG(INFO) << "Update the valid chunk " << chunkId << " of file "
                         << fileMeta.name << " of versoin " << fileMeta.version
                         << " in container " << containerId
                         << " in the file metadata.";
              // if the chunk is valid, commit the chunk by updating the file
              // metadata
              ...
              LOG(INFO) << "Going to delete the invalid chunk "
                        << reqFileMeta.chunks[0].getChunkName() << " of file "
                        << reqFileMeta.name << " of versoin "
                        << reqFileMeta.version << " in container "
                        << reqFileMeta.containerIds[0] << ".";
              chunkIndicators[0] = true;
              removeJournal = self->_tcChunkManager->deleteFile(
                  reqFileMeta, chunkIndicators);
            }
          }
        } else {
          // re-issue the deletion request and mark the journal as to-remove if
          // succeeded
          LOG(INFO) << "Going to delete the chunk " << chunkId << " of file "
                    << fileMeta.name << " of versoin " << fileMeta.version
                    << " in container " << containerId << " again.";
          bool chunkExistsAndMatches =
              fileMeta.numChunks > chunkId &&
              fileMeta.containerIds[chunkId] == containerId &&
              fileMeta.chunks[chunkId].matchMeta(chunk);
          if (chunkExistsAndMatches) {
            removeJournal = true;
          } else {
            chunkIndicators[0] = true;
            removeJournal =
                self->_tcChunkManager->deleteFile(reqFileMeta, chunkIndicators);
          }
        }

        // remove the journal record once verified the chunk integrity or
        // successfully removed the invalid chunk
        if (removeJournal) {
          if (!self->_metastore->updateChunkInJournal(
                  fileMeta, chunk, isWrite, /* deleteRecord */ removeJournal,
                  containerId)) {
            LOG(WARNING) << "Failed to remove the chunk journal of file "
                         << file.name << " chunk " << chunkId
                         << " in namespace " << file.namespaceId << " version "
                         << file.version << ".";
          }
        }
      }
...
void *Proxy::stagingBGWrite(void *param) {
  ...
  while (self->_running && scanIntv > 0) {
    ...
    if (bgwritePolicy == "none") {
      ...
      std::string bgwriteTimestamp =
          config.getProxyStagingBackgroundWriteTimestamp();
      ...
      scanIntv = deadlineToday +
                 ((deadlineToday <= time(NULL)) ? 24 * 60 * 60 : 2) - lastScan;
    ...
    if (!(lastScan == -1 || lastScan + scanIntv <= time(NULL))) {
      ...
      DLOG(INFO) << BG_WRITE_TO_CLOUD_TAG << "Sleep until"
                 << (bgwritePolicy == "immediate" ? " next foreground write or"
                                                  : "")
                 << " next background write scan at " << ctime(&nextScanSec);
      pthread_cond_timedwait(&self->_stagingBgWritePending,
                             &self->_stagingBgWritePendingLock, &nextScan);
    ...
    if (bgwritePolicy == "idle") {
      // Current implementation assumes the system is idle when cpu utilization
      // is below 50%
      SysInfo info;
      ...
      // if (!(avgCPUUsage / info.cpu.num < 50) && (info.mem.free * 1.0 /
      // info.mem.total < 0.5)) {
    ...
    while (self->_running) {
      ...
      if (self->bgwriteFileToCloud(*wf)) {
        LOG(INFO) << BG_WRITE_TO_CLOUD_TAG
                  << "Background write task added, file: " << wf->name;
      } else {
        LOG(ERROR) << BG_WRITE_TO_CLOUD_TAG
                   << "Failed to add background write task, file: " << wf->name;
      }

      // TODO rate limiting (avoid overload)
    }

    // update time of last scan (at the end of scan, to avoid scan interval <
...
bool Proxy::bgwriteFileToCloud(File &f) {
  ...
  int scanIntv =
      Config::getInstance().getProxyStagingBackgroundWriteScanInterval();
  if (!staged || rfinfo.mtime + scanIntv * 2 > time(NULL)) {
    _metastore->markFileAsPendingWriteToCloud(f);
    LOG(WARNING) << BG_WRITE_TO_CLOUD_TAG << "Skip "
                 << (staged ? "recently modified" : "non-existing") << " file "
                 << rf.name;
    return false;
  }
  ...
  if (lockFileAndGetMeta(rf, "background write") == false) {
    _metastore->markFileAsPendingWriteToCloud(f);
    LOG(ERROR) << BG_WRITE_TO_CLOUD_TAG << "Failed to get metadata of file "
               << rf.name;
    return false;
  }
  ...
  // get stripe size
  unsigned long stripeSize = _chunkManager->getMaxDataSizePerStripe(
      codingMeta.coding, codingMeta.n, codingMeta.k, codingMeta.maxChunkSize);
  if (stripeSize == INVALID_FILE_OFFSET) {
    _metastore->markFileAsPendingWriteToCloud(f);
    unlockFile(f);
    LOG(ERROR) << BG_WRITE_TO_CLOUD_TAG << "Failed to get stripe size for "
               << codingMeta.print();
    return false;
  }

  int numStripes =
      (fileSize / stripeSize) + (fileSize % stripeSize == 0 ? 0 : 1);

  // find the containers
  int *spareContainers = 0;
  int numSelected = 0; // no need to find selected containers
  if (prepareWrite(rf, wf, spareContainers, numSelected,
                   /* needsFindSpareContainers */ false) == false) {
    _metastore->markFileAsPendingWriteToCloud(f);
    LOG(ERROR) << BG_WRITE_TO_CLOUD_TAG
               << "Failed to prepare for background write for file " << rf.name;
    unlockFile(f);
    return false;
  }
  ...
  if (!_staging->readFile(wf)) {
    _metastore->markFileAsPendingWriteToCloud(f);
    unlockFile(f);
    LOG(ERROR) << "Failed to read file " << f.name
               << " from staging for write back to cloud";
    return false;
  }

  boost::timer::cpu_times duration = mytimer.elapsed();
  LOG(INFO) << "Read file " << f.name << " from staging, (data) speed = "
            << (wf.length * 1.0 / (1 << 20)) / (duration.wall * 1.0 / 1e9)
            << " MB/s "
            << "(" << wf.size * 1.0 / (1 << 20) << "MB in "
  ...
  LOG_IF(INFO, duration.wall > 0)
      << "Write back file " << f.name << " to cloud, (data) speed = "
      << (wf.length * 1.0 / (1 << 20)) / (duration.wall * 1.0 / 1e9) << " MB/s "
      << "(" << wf.size * 1.0 / (1 << 20) << "MB in "
      << (duration.wall * 1.0 / 1e9) << " seconds)";
  ...
  LOG(INFO) << "Write back file " << f.name
            << ", (meta) duration = " << (mytimer.elapsed().wall * 1.0 / 1e6)
            << " millseconds)";
  ...
  const std::map<std::string, double> stats =
      genStatsMap(duration, mytimer.elapsed(), wf.size);
  _statsSaver.saveStatsRecord(stats, "write staged file",
                              std::string(wf.name, wf.nameLength), start,
                              time(NULL));
  return true;
}

std::map<std::string, double>
Proxy::genStatsMap(const boost::timer::cpu_times &dataT,
                   const boost::timer::cpu_times &metaT,
                   const unsigned long int &dataSize) const {
  std::map<std::string, double> stats;

  stats["meta (ms)"] = metaT.wall * 1.0 / 1e6;
  stats["data (s)"] = dataT.wall * 1.0 / 1e9;
  stats["data (MB/s)"] =
      (dataSize * 1.0 / (1 << 20)) / (dataT.wall * 1.0 / 1e9);
  stats["fileSize"] = (dataSize * 1.0 / (1 << 20));

  return stats;
}

Proxy::StripeLocation::StripeLocation() { reset(); }

Proxy::StripeLocation::StripeLocation(const std::string name,
