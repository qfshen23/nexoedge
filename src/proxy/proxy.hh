...
public:
  Proxy();
  Proxy(ProxyCoordinator *coordinator, std::map<int, std::string> *map,
        BgChunkHandler::TaskQueue *queue = 0, DeduplicationModule *dedup = 0,
        bool enableAutoRepair = Config::getInstance().autoFileRecovery());
  virtual ~Proxy();
   ...
   *
   * @param[in] f file to write, containing name, size, offset (0), length (same
   *as size) and data
   *
   ...
   *
   * @param[in,out] f file to append, containing name, offset (needs to be a
   *multiple of stripe size), length (needs to be a multiple of stripe size),
   *and data; size is updated to the current file size upon successful overwrite
   *
   ...
   *
   * @param[in,out] f file to append, containing name, offset (needs to be a
   *multiple of stripe size), length (needs to be a multiple of stripe size, or
   *less only if this is the last append), and data; size is updated to the
   *current file size upon successful append
   *
   ...
   *
   * @param[in] f file to read, containing the name, the size and data will be
   *returned on successful read
   *
   ...
   * @param[out] f      file to read, the name, size and data will be returned
   *on successful read
   *
   * @return whether the read is successful
   **/
   ...
   *
   * @param[in] f file to read, containing the name, offset and length; data
   *will be returned on successful read
   *
   * @return whether the read is successful
   **/
   ...
   * @param[out] f      file to delete, the name, size and data will be returned
   ...
   * @param[in] sf     file to copy, containing the name
   * @param[in,out] df copied file, containing the name; with a new file id
   *assigned upon copy success
   *
   * @return whether the copying is successful
   **/
  ...
  getExpectedAppendSize(const std::string storageClass);
   ...
   * @return the expected size of data for read
   **/
  virtual unsigned long int getExpectedReadSize(boost::uuids::uuid fuuid,
                                                File &f);
  ...
  getFileList(FileInfo **list, bool withSize = true, bool withVersions = false,
              unsigned char namespaceId = INVALID_NAMESPACE_ID,
              std::string prefix = "");
   ...
   **/
  virtual unsigned int
  getFolderList(std::vector<std::string> &list,
                unsigned char namespaceId = INVALID_NAMESPACE_ID,
                std::string prefix = "");
   ...
   **/
  virtual void getFileCountAndLimit(unsigned long int &count,
   ...
   **/
  virtual bool getNumFilesToRepair(unsigned long int &count,
                                   unsigned long int &repair);
   ...
   **/
  virtual void getStorageUsage(unsigned long int &usage,
...
protected:
  ...
  bool prepareWrite(File &f, File &wf, int *&spareContainers, int &numSelected,
                    bool needsFindSpareContainers = true);

  /**
   * Write file stripes
   * @param[in] f                      current base file struct for
   *write/overwrite/append
   * @param[in,out] wf                 file containing stripes to write
   * @param[in] spareContainers        id of containers which are
   *spared/selected for write
   * @param[in] numSelected            number of containers in spareContainers
   *
   * @return whether the stripes in wf are written sucessfully
   **/
  bool writeFileStripes(File &f, File &wf, int spareContainers[],
                        int numSelected);
  ...
  virtual unsigned long int getExpectedAppendSize(int codingScheme, int n,
  ...
  bool sortStripesAndBlocks(
      ...
      std::vector<Fingerprint> &duplicateBlockFps, int dataStripeSize = -1);
   ...
   * @param[in,out] numChunksInBatch    number of chunks in the current batch
   *(info. across consecutive calls)
   * @param[in,out] batchStartIdx       starting file index of the batch (info.
   *across consecutive calls)
   *
   * @return whether the check or scan encountered an error
   **/
  bool batchedChunkScan(const FileInfo *list, const int numFiles,
   ...
   * @param[in,out] chunkIndicator      indicators which indicate failed chunks
   *(as false) on input, and indicate both failed and corrupted chunks (as
   *false) on output
   *
   * @return the number of corrupted but not failed chunks
   **/
  int checkCorruptedChunks(bool *chunksCorrupted, int numChunks,
                           bool *chunkIndicator);
  ...
  genStatsMap(const boost::timer::cpu_times &dataT,
  ...
  std::map<int, std::string>
      *_containerToAgentMap;      /**< map of containers [container id]->agent
                                     socket*/
  ...
  pthread_t
      _stagingBGWriteWorker; /**< staging background write to cloud worker */
  pthread_t _stagingBGCacheReadWorker; /**< staging background stage read cache
                                          worker */
  pthread_cond_t
      _stagingBgWritePending; /**< staging background write pending condition*/
  pthread_mutex_t
      _stagingBgWritePendingLock; /**< staging background write pending lock */
  RingBuffer<File>
      *_stagingPendingReadCache; /**< staging background read cache buffer */
};
