...
bool Proxy::writeFile(File &f) {
  ...
  if (_metastore->getMeta(of)) {
    ...
    // delete old file chunks only when (i) system is configured to overwrite
  ...
  //    if (!_metastore->updateChunkInJournal(wf, chunk, /* isWrite */ true, /*
  //    deleteRecord */ true, containerId)) {
  //        LOG(ERROR) << "Failed to journal the chunk change of file " <<
  //        wf.name << " chunk " << chunk.getChunkId() << ".";
  //    }
  //}

  removeOldData.start();
  // if the new data is written to backend (not staging), one can safely remove
...
bool Proxy::modifyFile(File &f, bool isAppend) {
  ...
  if (((_stagingEnabled && of.staged.storageClass != f.storageClass) ||
  ...
  if (_staging && of.staged.size > 0 && of.staged.mtime >= of.mtime) {
    ...
    // only report failure if staged file is more recent, but not visible to
    // this proxy for modifications if file on cloud is as recent as the staged
  ...
  if (prepareWrite(of, wf, spareContainers, numSelected,
                   /* needsFindSpareContainers */ false) == false) {
    unlockFile(of);
    of.name = 0;
    // swap the information back
  ...
  // offset the address calculation, since the data is assumed to be pointing at
  // the start of file
...
#define CLEAN_UP_PREVIOUS_STRIPES(__END_IDX__)                                                 \
  do {                                                                                         \
    ...
    // add commit id to file (do it here instead of after chunk write, so if
    // returned on error, the current commit id can also be aborted)
...
bool Proxy::readFile(File &f, bool isPartial) {
  ...
  readData.start();
  // handle file with staged copy, read from the staged copy if it exists and is
  // most recent
  FileInfo rinfo;
...
#define clean_external_filemeta()                                               \
  ...
  for (int i = startStripe; i < endStripe; i++, currStripeId++) {
    ...
    if (useTempBuffer) {  // copy data back to the original file data buffer
      ...
      // reset to tmp buffer pointer to avoid double free
    ...
    // unset the data reference to the original file data buffer or the temp
    // buffer
    srf.data = 0;
...
bool Proxy::sortStripesAndBlocks(
  ...
  for (size_t i = 0; i < numDuplicateBlocks; i++, dit++) {
    ...
    if (fit == externalFiles.end()) {
      ...
      // DLOG(INFO) << "Duplicated block(s) to be read from an external object "
      // << extFilename << " in namespace " << (int)
      // blockLoc.getObjectNamespaceId() << " version " << ef->version;
    } else {
    ...
    try {
      ...
      // mapping (object name, logical offset, physical in-stripe offset)
      ...
      LOG(ERROR) << "Cannot find the physcial location of a duplicated block "
                    "in the source file "
                 << extFilename << " at offset " << extStripeOffset;
      // clean up
...
bool Proxy::deleteFile(const File &f) {
  ...
  //    if (!_metastore->addChunkToJournal(df, df.chunks[cidx], containerId, /*
  //    isWrite */ false)) {
  //        LOG(ERROR) << "Failed to journal file " << df.name << " deletion of
  //        chunk " << cidx << " in container " << containerId << ".";
  //        unlockFile(df);
  //        return false;
  //    }
  //}
...
bool Proxy::repairFile(const File &f, bool isBg) {
  ...
  mytimer.start();
  // for (int i = 0; i < rf.numChunks; i++) DLOG(INFO) << "Chunk " << i << "
  // container = " << rf.containerIds[i];
  ...
  for (int i = 0; i < rf.numStripes; i++) {
    ...
    //    if (!chunkIndicator[cidx] && srf.containerIds[cidx] ==
    ...
    for (int i = 0; i < numChunkGroups; i++)
        for (int j = 0; j < chunkGroups[i * (rf.numChunks + 1)]; j++)
            DLOG(INFO) << "Group " << i << " chunk " << j << " id = " <<
    chunkGroups[i * (rf.numChunks + 1) + j + 1];
    */
    // repair the chunks
    if ((!isBg &&
...
bool Proxy::unlockFile(const File &f) { return _metastore->unlockFile(f); }
