        ...
        rabin_polynomial_min_block_size_(RABIN_MIN_BLOCK_SIZE),
{
    ...
    if(polynomial_lookup_buf_ == nullptr) {
        LOG(ERROR) << "Could not initialize rabin polynomial lookup buffer, out of memory!\n";
        return;
    }
 ...
 */
struct rabin_polynomial* RabinChunker::gen_new_polynomial(struct rabin_polynomial *tail, uint64_t total_len, uint64_t length, uint64_t rab_sum) {
 ...
 */
struct rabin_block_info* RabinChunker::init_empty_block() {
	struct rabin_block_info* block = malloc(sizeof(struct rabin_block_info));
    if(block == nullptr) {
    ...
    block -> cur_window_data = malloc(sizeof(char) * rabin_sliding_window_size_);
	...
	for(int i = 0;i < rabin_sliding_window_size_;i ++) {
 ...
 */
struct rabin_block_info* RabinChunker::read_rabin_block(const void *buf, ssize_t size, struct rabin_block_info *cur_block) {
    ...
    for(ssize_t i = 0;i < size;i ++) {
        ...
        block -> cur_roll_checksum -= (pushed_out * polynomial_lookup_buf[rabin_sliding_window_size_]);    
        block -> window_pos ++;
        block -> total_bytes_read ++;
        block -> tail->length ++;

        // cur_window_data is a ring buffer
        // if reach the end of cur_window_data, then loop back around to the start position
        ...
        if((block -> tail -> length >= rabin_polynomial_min_block_size_ && (block -> cur_roll_checksum % rabin_polynomial_average_block_size_) == rabin_polynomial_prime_)
