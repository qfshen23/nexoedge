...
so all functions in this file should be considered as interfaces for data
deduplication.
...
public:
  DeduplicationModule() { _chunker = 0; }

  virtual ~DeduplicationModule() { delete _chunker; };

  /**
   * Scan buffer for unique and duplicated data
   *
   * @param[in] data                     data buffer
   * @param[in] dataInObjectLocation     location of data in the object, with
   *object namespace id, object name, object version, in-object data offset, and
   *data length specified
   * @param[out] blocks                  the list of blocks (ordered by offset),
   *with their in-object offset, length, corresponding fingerprints and whether
   *they are a duplicate of existing blocks specified
   *
   * @return commit id for the list of blocks
   **/

  virtual std::string
  scan(const unsigned char *data, const BlockLocation &dataInObjectLocation,
   ...
   *
   * @param[in] commitId                 commit id returned by the scan() for
   *the list of blocks to commit
   *
   **/
   ...
   **/
  virtual std::vector<BlockLocation>
  query(const unsigned char namespaceId,
        const std::vector<Fingerprint> &fingerprints) = 0;
   ...
   **/
  virtual std::string
  update(const std::vector<Fingerprint> &fingerprints,
         const std::vector<BlockLocation> &oldLocations,
         const std::vector<BlockLocation> &newLocations) = 0;
